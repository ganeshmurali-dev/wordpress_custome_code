...
add_action( 'wp_enqueue_scripts', 'yuki_enqueue_woo_scripts', 0 );
		...
		if ( yuki_is_woo_shop() ) {
			...
			$css['.woocommerce .yuki-products li.product .yuki-product-wrapper'] = array_merge(
				...
				[
					...
					'--card-content-vertical-alignment' => CZ::get( 'yuki_store_card_vertical_alignment' ),
...
if ( ! function_exists( 'yuki_woo_before_content' ) ) {
	...
	function yuki_woo_before_content() {
		...
		<?php if ( ! is_shop() ): ?>
            <div class="yuki-article-content yuki-entry-content clearfix mx-auto">
		<?php endif; ?>
		<?php
	}
}
...
if ( ! function_exists( 'yuki_woo_archive_before_description' ) ) {
	function yuki_woo_archive_before_description() {
		?>
...
if ( ! function_exists( 'yuki_woo_archive_after_description' ) ) {
	function yuki_woo_archive_after_description() {
		?>
        </div>
		<?php
	}
...
if ( ! function_exists( 'yuki_modify_woo_template_hooks' ) ) {
	...
	function yuki_modify_woo_template_hooks() {
		...
		// Change store catalog description wrap
		add_action( 'woocommerce_archive_description', 'yuki_woo_archive_before_description', 0 );
		add_action( 'woocommerce_archive_description', 'yuki_woo_archive_after_description', 999 );
