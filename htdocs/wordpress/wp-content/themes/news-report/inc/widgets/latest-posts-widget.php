<?php
if ( ! class_exists( 'News_Report_Latest_Posts_Widget' ) ) {
	/**
	 * Adds News Report Latest Posts Widget.
	 */
	class News_Report_Latest_Posts_Widget extends WP_Widget {
		...
		public function __construct() {
			$latest_posts_widget = array(
				...
				'description' => __( 'Retrive Latest Posts Widgets', 'news-report' ),
			);
			parent::__construct(
				'news_report_latest_posts_widget',
				__( 'Adore Widget: Latest Posts Widget', 'news-report' ),
				$latest_posts_widget
			);
		}
		...
		*/
		public function widget( $args, $instance ) {
			...
			$latest_posts_post_count  = isset( $instance['number'] ) ? absint( $instance['number'] ) : 4;
			...
			if ( ! empty( $latest_posts_title || $viewall_button_label ) ) {
				?>
			...
			<div class="adore-widget-body">
				<div class="post-latest-widget-wrapper">
					...
					$latest_posts_widgets_args = array(
						'post_type'      => 'post',
						'posts_per_page' => absint( $latest_posts_post_count ),
		...
		public function form( $instance ) {
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>"><?php esc_html_e( 'Number of posts to show(Min:1 | Max:4):', 'news-report' ); ?></label>
				<input class="tiny-text" id="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'number' ) ); ?>" type="number" step="1" min="1" max="4" value="<?php echo absint( $latest_posts_post_count ); ?>" size="3" />
			</p>
			<p>
