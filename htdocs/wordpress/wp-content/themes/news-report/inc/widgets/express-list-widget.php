<?php
if ( ! class_exists( 'News_Report_Express_List_Widget' ) ) {
	/**
	 * Adds News Report Express List Widget.
	 */
	class News_Report_Express_List_Widget extends WP_Widget {
		...
		public function __construct() {
			$express_list_widget = array(
				'classname'   => 'widget adore-widget express-list-widget style-1',
				'description' => __( 'Retrive Express List Widgets', 'news-report' ),
			);
			parent::__construct(
				'news_report_express_list_widget',
				__( 'Adore Widget: Express List Widget', 'news-report' ),
				$express_list_widget
			);
		}
		...
		public function widget( $args, $instance ) {
			...
			$express_list_post_count  = isset( $instance['number'] ) ? absint( $instance['number'] ) : 8;
			...
			if ( ! empty( $express_list_title || $viewall_button_label ) ) {
				?>
			...
			<div class="adore-widget-body">
				<div class="express-list-widget-wrapper">
					...
					$express_list_widgets_args = array(
						'post_type'      => 'post',
						'posts_per_page' => absint( $express_list_post_count ),
						...
						while ( $query->have_posts() ) :
							...
							<div class="post-item <?php echo esc_attr( $class ); ?>">
								...
								<div class="post-item-content">
									<div class="entry-cat <?php echo esc_attr( $cat_bg_color ); ?>">
									...
									<?php if ( $i === 1 ) : ?>
										<div class="post-exerpt">
											...
											$content = wp_kses_post( wp_trim_words( $content, 15 ) );
											?>
		...
		public function form( $instance ) {
			...
			?>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Section Title:', 'news-report' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'view_all_button_label' ) ); ?>"><?php esc_html_e( 'View All Button:', 'news-report' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'view_all_button_link' ) ); ?>"><?php esc_html_e( 'View All Button URL:', 'news-report' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>"><?php esc_html_e( 'Number of posts to show(Min:1 | Max:8):', 'news-report' ); ?></label>
				<input class="tiny-text" id="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'number' ) ); ?>" type="number" step="1" min="1" max="8" value="<?php echo absint( $express_list_post_count ); ?>" size="3" />
			</p>
			<p>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'category' ) ); ?>"><?php esc_html_e( 'Select the category to show posts:', 'news-report' ); ?></label>
