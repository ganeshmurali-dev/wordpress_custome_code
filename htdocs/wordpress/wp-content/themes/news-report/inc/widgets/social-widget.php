<?php
if ( ! class_exists( 'News_Report_Social_Widget' ) ) {
	/**
	 * Adds News Report Social Widget.
	 */
	class News_Report_Social_Widget extends WP_Widget {

		/**
		 * Register widget with WordPress.
		 */
		public function __construct() {
			$news_report_social_widget = array(
				'classname'   => 'widget adore-widget social-widget style-2',
				'description' => __( 'Retrive Social Widget', 'news-report' ),
			);
			parent::__construct(
				'news_report_social_widget',
				__( 'Adore Widget: Social Widget', 'news-report' ),
				$news_report_social_widget
			);
		}
		...
		public function form( $instance ) {
			...
			for ( $i = 1; $i <= 4; $i++ ) {
				...
				?>
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'social_icon_label-' . $i ) ); ?>"><?php echo sprintf( esc_html__( 'Social Icon Label %d :', 'news-report' ), $i ); ?></label>
				...
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'link-' . $i ) ); ?>"><?php echo sprintf( esc_html__( 'Link %d :', 'news-report' ), $i ); ?></label>
