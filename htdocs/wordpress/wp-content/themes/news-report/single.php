...
<main id="primary" class="site-main">
	...
	while ( have_posts() ) :
		...
		the_post_navigation(
			array(
				'prev_text' => '<span class="nav-subtitle">' . esc_html__( 'Previous:', 'news-report' ) . '</span> <span class="nav-title">%title</span>',
				'next_text' => '<span class="nav-subtitle">' . esc_html__( 'Next:', 'news-report' ) . '</span> <span class="nav-title">%title</span>',
			)
		);
		...
		if ( is_singular( 'post' ) ) {
			...
			if ( $query->have_posts() ) :
				$related_title = get_theme_mod( 'news_report_related_posts_title', __( 'Related Posts', 'news-report' ) );
				?>
							...
							<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
								<div class="post-item post-grid">
									<div class="post-item-image">
										<?php news_report_post_thumbnail(); ?>
									</div>
	...
	if ( news_report_is_sidebar_enabled() ) {
		get_sidebar();
	}
